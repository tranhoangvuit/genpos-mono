...
type TabKey = 'all' | 'draft' | 'submitted' | 'partial' | 'received' | 'cancelled'
type SortKey = 'po' | 'supplier' | 'status' | 'total' | 'arrival'
...
function statusPalette(s: string) {
  switch (s) {
    ...
    case 'partial':
      return { bg: LOW_SOFT, color: LOW_INK }
    case 'received':
...
function statusLabel(s: string) {
  if (s === 'submitted') return 'Ordered'
  if (s === 'cancelled') return 'Closed'
  return s.charAt(0).toUpperCase() + s.slice(1)
}

function formatDate(ts: PurchaseOrderListRow['expectedAt']) {
  if (!ts) return '—'
  try {
    ...
    arr = [...arr].sort((a, b) => {
      ...
      switch (sortKey) {
        ...
        case 'total':
          ...
          break
        case 'arrival': {
          const ax = a.expectedAt ? Number(a.expectedAt.seconds) : 0
          const bx = b.expectedAt ? Number(b.expectedAt.seconds) : 0
          cmp = ax - bx
          break
        }
        default:
      ...
      >
        ...
        >
          <div className="flex items-center gap-0.5">
            {(['all', 'draft', 'submitted', 'partial', 'received', 'cancelled'] as TabKey[]).map(
              (k) => (
                <button
                  key={k}
                  type="button"
...
function Row({
  ...
  return (
    <tr className="transition hover:bg-[hsl(210_40%_96%_/_0.3)]">
      ...
      <Td>
        <span className="tabular-nums">{r.itemCount > 0 ? `0 of ${r.itemCount}` : '—'}</span>
      </Td>
      <Td align="right">
        <span className="tabular-nums">{formatMoney(r.total)}</span>
      </Td>
      <Td>
        <span style={{ color: MUTED_FG }}>{formatDate(r.expectedAt)}</span>
      </Td>
    </tr>
  )
}
