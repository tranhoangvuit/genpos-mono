...
import type { CsvProductRow } from '@/gen/genpos/v1/catalog_pb'
...
import { useImportProducts, useParseImportCsv } from './hooks'
...
export function ImportProductDialog({ open, onOpenChange }: Props) {
  ...
  const parseMut = useParseImportCsv()
  ...
  return (
    <Dialog open={open} onOpenChange={close}>
      <DialogContent className="max-w-3xl">
        <DialogHeader>
          <DialogTitle>{t('catalog.importProducts')}</DialogTitle>
          <DialogDescription>{t('catalog.importSubtitle')}</DialogDescription>
        </DialogHeader>
        ...
        {preview && !result && (
          <div className="space-y-3">
            ...
            <DialogFooter>
              ...
              <Button type="button" onClick={onImport} disabled={importing || preview.validCount === 0}>
                {importing ? t('common.importing') : t('catalog.importNow', { n: preview.validCount })}
              </Button>
            </DialogFooter>
          </div>
        )}
